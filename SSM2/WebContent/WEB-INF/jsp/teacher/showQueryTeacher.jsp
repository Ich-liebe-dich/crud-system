...
<div class="container">
  <div class="card">
    ...
    <div class="card-body">
    	<a href="${ctx}/excel/teacher?name=teacherExcel">导出</a>
    	 <form method="post" action="${ctx}/teacher/queryTeacher/1" id="searchForm">
  			教师编号：<input type="number" name="id" id="id" value="${requestScope.queryTeacher.id }">
  			姓名：<input name="name" id="name" value="${requestScope.queryTeacher.name }">
  			<c:choose>
			    <c:when test="${requestScope.queryTeacher.sex eq '男' }">
			    ...
			    <c:when test="${requestScope.queryTeacher.sex eq '女' }">
		  ...
		  <table class="table table-hover">
		    ...
		    <tbody>
		      <c:forEach items="${requestScope.teacherList}" var="teacher" varStatus="stat">
		      	 <tr>
		      	      <td> <input type="checkbox" id="box_${stat.index}" value="${teacher.id}"></td>
			        <td>${teacher.id}</td>
			        <td>${teacher.name}</td>
			        <td>${teacher.sex}</td>
			        <td>
			        	<c:forEach items="${teacher.courseList}" var="course">
					        ${course.name}&nbsp;&nbsp;
				        </c:forEach>
			        </td>
			         <td><a href="${ctx}/teacher/updateTeacher?id=${teacher.id}">修改</a></td>
			      </tr>
		      </c:forEach>      
		     </tbody>
		  </table>
...
$(function(){
	$("#pageIndex").bind('keypress',function(event){ 
        ...
        if(event.keyCode == 13) { 
        	if((index <= ${requestScope.page.totalSize}) && index >= 1){
        		$('form[id=searchForm]').attr('action',$("#PageContext").val()+'/teacher/queryTeacher/'+index);
            	$("#searchForm").submit();
        	}else{
        		alert("请输入正确的页码！");
        	}
    ...
    $("#last").click(function(){
    	...
    	$('form[id=searchForm]').attr('action',$("#PageContext").val()+'/teacher/queryTeacher/'+last);
    	$("#searchForm").submit();
    })
     $("#next").click(function(){
    	var index = $("#index").val();
    	...
    	$('form[id=searchForm]').attr('action',$("#PageContext").val()+'/teacher/queryTeacher/'+next);
    	$("#searchForm").submit();
    })
	 		   ...
	 		   if(checkedBoxs.length < 1){
	 			  alert("请选择一个需要删除的用户！");
	 			   ...
	 			   if(confirm("删除后无法复原，确认删除吗？")){
 					   // 发送请求
 					   window.location =  $("#PageContext").val()+"/teacher/deleteTeacher?ids=" + ids.get();
	 			   }
...
/**/
function check(){
	if(typeof($("#id").val())=="undefined" &&typeof($("#name").val())=="undefined"&&(typeof($("#sex1").val())=="undefined" || typeof($("#sex2").val())=="undefined") ){
		window.location.href = $("#PageContext").val()+"/teacher/selectTeacher/1";
		return false;	
	}
	return true;
}
